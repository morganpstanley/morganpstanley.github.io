...
const Project = ({ project }: any): JSX.Element => {
  ...
  const showBullets = PROJECT_BULLETS.map((bullet: string, i: number) => (
    <p key={i}>{bullet}</p>
  ));

  const showImages = PROJECT_IMAGES.map((image: string, i: number) => (
    <img loading="lazy" key={i} src={image} alt="Portfolio Project" />
  ));

  const showProjectName = (projectName: string, link: string | undefined) => {
    if (link)
      return (
        <a href={link} className="project-link font-playfair">
          {projectName}⤤
        </a>
      );
  ...
  return (
    <>
      ...
      <div className="project-details">
        <Carousel {...getConfigurableProps()}>{showImages}</Carousel>
        <div id="project-description">{showBullets}</div>
      </div>
    </>
  );
};
